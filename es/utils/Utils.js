...
export class Utils {
    constructor() {
        ...
        $(document).on("click", "[data-stockclick]", function (e) {
            NativeJs.gotoStockDetailPage($(this).text());
        });
    }
    UUID() {
        function s4() {
    ...
    textOfStockcodeOnClick(text) {
        ...
        text = text.replace(/((002|000|300|600|601|603|60[\d]{1})[\d]{3})/, function (match, param1) {
            let astock = `<a class="stockcode" data-stockclick="${match}">${match}</a>`;
            return astock;
        });
        return text;
    }
    getAppVersion() {
        if (this.isApp()) {
            const ua = navigator.userAgent.toLowerCase();
            const version = ua.match(/hnversion[\d].[\d].[\d]/ig);
            if (version) {
                return version[0];
            }
            return false;
