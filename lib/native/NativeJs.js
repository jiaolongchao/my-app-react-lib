...
var NativeJs = /** @class */ (function () {
    ...
    NativeJs.prototype.refreshtoken_load = function () {
        ...
        else if (Utils_1.default.isJDJR()) {
        }
        else {
    ...
    NativeJs.prototype.toPay = function (ref_id, ref_type, buyCycle) {
        ...
        if (Utils_1.default.isApp()) {
            ...
            Utils_1.default.redirectPay(ref_id, ref_type, buyCycle);
        }
    };
    /**
     * 应用内部跳转
    ...
    NativeJs.prototype.getPayInfo = function (messageBody, callback) {
        ...
        var paymethod = messageBody.paymethod, prepay = messageBody.prepay;
        if (!Utils_1.default.isApp() || Utils_1.default.isIOS()) {
            // callback(true)
            if (paymethod === 2) {
                var ali = "https://openapi.alipay.com/gateway.do?" + prepay;
                location.href = ali;
            }
            else if (paymethod === 1) {
                var prepayobj = JSON.parse(prepay);
     ...
     * 直接调起适当性认证
     */
    NativeJs.prototype.startFsp = function (productId, riskScore, order_id) {
        if (Utils_1.default.isApp()) {
            exports.baseNativeJs("startFsp", { productId: productId, riskScore: riskScore, order_id: order_id });
        }
        else {
            Utils_1.default.redirectFsp(productId, riskScore, order_id);
        }
    };
    /**
     * 线下支付提示
    ...
    NativeJs.prototype.webPSucceeded = function () {
        exports.baseNativeJs("webPSucceeded");
    };
    /**
     * 调用原生的下拉刷新
     */
    NativeJs.prototype.pullRefresh = function (pullRefresh, callback) {
        window['pullRefresh'] = function () {
            if (callback) {
                callback();
            }
        };
        exports.baseNativeJs("pullRefresh", { pullRefresh: pullRefresh });
    };
    /**
     * 原生页面返回的时候，回调方法
     */
    NativeJs.prototype.appBackReload = function (appBackReload, callback) {
        window['appBackReload'] = function () {
            if (callback) {
                callback();
            }
        };
        exports.baseNativeJs("appBackReload", { appBackReload: appBackReload });
    };
    return NativeJs;
}());
exports.NativeJs = NativeJs;
var intance = new NativeJs();
exports.default = intance;
if (window) {
